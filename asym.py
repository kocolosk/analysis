...
Created by Adam Kocoloski on 2007-04-23.
...
class AsymmetryGenerator:
   ...
   def __init__(self, name, usePolarizations = True, useR123 = True, bins = [4, 2.0, 10.0]):
      self.name = name
      self.bins = bins
      if len(self.bins) == 3:
         self.top = { 
            'ls' : ROOT.TH1D('_%s_ls' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
            'us' : ROOT.TH1D('_%s_us' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
            'ly' : ROOT.TH1D('_%s_ly' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
            'lb' : ROOT.TH1D('_%s_lb' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
            'll' : ROOT.TH1D('_%s_ll' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
         }
         self.bot = { 
            'ls' : ROOT.TH1D('_%s_ls_bot' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
            'us' : ROOT.TH1D('_%s_us_bot' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
            'ly' : ROOT.TH1D('_%s_ly_bot' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
            'lb' : ROOT.TH1D('_%s_lb_bot' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
            'll' : ROOT.TH1D('_%s_ll_bot' % (name,), '', self.bins[0], self.bins[1], self.bins[2]),
         }
      else:
         arraybins = array('d')
   ...
   def FillFromHistogramManager(self, mgr, trigId, charge, uu, ud, du, dd, Py, Pb):
      self.FillFromHistogram(5, uu, ud, du, dd, Py, Pb, mgr['uu'][trigId].trackHistograms(charge)['pt'])
      self.FillFromHistogram(6, uu, ud, du, dd, Py, Pb, mgr['du'][trigId].trackHistograms(charge)['pt'])
      self.FillFromHistogram(9, uu, ud, du, dd, Py, Pb, mgr['ud'][trigId].trackHistograms(charge)['pt'])
      self.FillFromHistogram(10,uu, ud, du, dd, Py, Pb, mgr['dd'][trigId].trackHistograms(charge)['pt'])
   ...
   def _FillR456(self, spinbit, uu, ud, du, dd, Py, Pb, val):
      R4 = float(dd)/uu ## NB: these might not be the traditional definitions of R4/5/6
      R5 = float(dd)/ud
      R6 = float(dd)/du
...
class polarizations:
    def __init__(self,year,version):
        path = {'2005_online':'/Users/kocolosk/data/run5/online_polarizations.txt',
...
def compareTheoryCurves(charge='plus'):
    ...
    import xsec
    if charge == 'plus':
        dss_std = theoryCurves(werner_plus_dss_cteqm5_std, xsec.werner_plus_dss_cteqm5_pt).getGraph()
...
def initReaderRun6Deltag5(reader):
    reader.selectDataset('/Library/STAR/DEV/StRoot/StSpinPool/StSpinTree/datasets/run6_mit.dataset')
    ...
    reader.connectElectrons     = False
    reader.connectGammas        = False
    
    reader.selectRunlist('/Volumes/scratch/kocolosk/dropBox/commonlist.txt')
...
def initReaderRun5(reader):
        ...
        #reader.selectRun(6120015)
        reader.selectRunlist('/Users/kocolosk/data/run5/runlists/45.runlist')
        #reader.selectRunlist('/Users/kocolosk/Xcode/StarSpinLibraries/StRoot/StSpinPool/StSpinTree/filters/run5_chargedPions.runlist')
        #reader.selectRunlist('/Library/STAR/DEV/StRoot/StSpinPool/StSpinTree/filters/run5_jets.runlist')
        
        #fill 7032
        #reader.removeRun(6127035)
        #reader.removeRun(6127036)
        #reader.removeRun(6127037)
        #
        ##fill 7035
        #reader.removeRun(6128043)
        #reader.removeRun(6128043)
        #reader.removeRun(6128051)
        #reader.removeRun(6128052)
        #reader.removeRun(6128053)
        #reader.removeRun(6128054)
        #reader.removeRun(6128055)
        #reader.removeRun(6128056)
        #reader.removeRun(6128057)
        #reader.removeRun(6128058)
        #reader.removeRun(6128059)
        #reader.removeRun(6128062)
        #reader.removeRun(6128063)
...
def make(init,relLumPath,year,useTai,polVersion):
    reader = ROOT.StSpinTreeReader()
    init(reader)
    print 'reader initialized'
    
    relLum = ROOT.StTamuRelLum.instance(relLumPath)
    print 'relative luminosities initialized'
    
    asymPlusForFill = {}
    asymMinusForFill = {}
        
    asym_plus = ROOT.FastAsymmetryMaker('plus')
    asym_plus.useR123 = True
    asym_plus.usePolarizations = False
    asym_plus.SetBins(4,2.,10.)
    
    asym_minus = ROOT.FastAsymmetryMaker('minus')
    asym_minus.useR123 = True
    asym_minus.usePolarizations = False
    asym_minus.SetBins(4,2.,10.)
    print 'asymmetry makers initialized'
    
    pol = polarizations(year,polVersion)
    print 'polarizations initialized'
    ...
    for i in xrange(reader.GetEntries()):
        reader.chain().GetEntry(i)
        
        #for trig in ev.triggers():
        #    print trig.trigId(), trig.didFire(), trig.shouldFire()
        
        myFill = int(ev.fill())
                
        if useTai and tai.useBxing(ev.fill(), ev.bx7()) == 0: continue
         
        spinbit = ev.spin4usingBx48()
        timebin7 = relLum.getRelLum(ev.runId(),5,7)
        timebin8 = relLum.getRelLum(ev.runId(),5,8)
        timebin9 = relLum.getRelLum(ev.runId(),5,9)
        if timebin7 is None: continue
        
        uu = timebin7.uu + timebin8.uu + timebin9.uu
        ud = timebin7.ud + timebin8.ud + timebin9.ud
        du = timebin7.du + timebin8.du + timebin9.du
        dd = timebin7.dd + timebin8.dd + timebin9.dd
        
        trigEta = None
        trigPhi = None
        for jet in reader.jets():
            if jet.geomTrigger(137221) or jet.geomTrigger(137222):
                trigEta = jet.Eta()
                trigPhi = jet.Phi()
                break
        
        for pion in reader.chargedPions():
            dR = 0.
            if trigEta is not None:
                dR = math.sqrt((trigEta-pion.Eta())**2 + (trigPhi-pion.Phi())**2)
                
            #if dR < 1.5: continue
            
            if pion.charge() > 0:
                asym_plus.Fill(spinbit, uu, ud, du, dd, pol.yellow(myFill), pol.blue(myFill), pion.Pt())
                try:
                    asymPlusForFill[myFill].Fill(spinbit, uu, ud, du, dd)
                except KeyError:
                    asymPlusForFill[myFill] = ROOT.FastAsymmetryMaker('plus_%d' % (ev.fill()))
                    asymPlusForFill[myFill].useR123 = True
                    asymPlusForFill[myFill].usePolarizations = False
                    asymPlusForFill[myFill].SetBins(1,-1.,1.)    
                    asymPlusForFill[myFill].Fill(spinbit, uu, ud, du, dd)
            else:
                asym_minus.Fill(spinbit, uu, ud, du, dd, pol.yellow(myFill), pol.blue(myFill), pion.Pt())
                try:
                    asymMinusForFill[myFill].Fill(spinbit, uu, ud, du, dd)
                except KeyError:
                    asymMinusForFill[myFill] = ROOT.FastAsymmetryMaker('minus_%d' % (ev.fill()))
                    asymMinusForFill[myFill].useR123 = True
                    asymMinusForFill[myFill].usePolarizations = False
                    asymMinusForFill[myFill].SetBins(1,-1.,1.)
                    asymMinusForFill[myFill].Fill(spinbit, uu, ud, du, dd)
    
    print 'selected events processed in %f CPU / %f real seconds' % (timer.CpuTime(),timer.RealTime())
    
    th = theoryCurves()
    plusGraphs = [ th.getGraph('plus','std'), th.getGraph('plus','max'), th.getGraph('plus','zero'), th.getGraph('plus','min') ]
    minusGraphs = [ th.getGraph('minus','std'), th.getGraph('minus','max'), th.getGraph('minus','zero'), th.getGraph('minus','min') ]
    
    #we want the following histograms for each charge
    #single-spin checks -- y,b,l.s.,u.s., vs. fill and vs. pt (4x2 = 8)
    #double-spin -- just the final plot
    
    plusHistos = [  asym_plus.GetAsymmetry(0), asym_plus.GetAsymmetry(1), asym_plus.GetAsymmetry(2),
                    asym_plus.GetAsymmetry(3), asym_plus.GetAsymmetry(4),
                    ROOT.TH1D('plusFillsY','#pi^{+} #epsilon_{LY}',len(asymPlusForFill),0.5,len(asymPlusForFill)+0.5), 
                    ROOT.TH1D('plusFillsB','#pi^{+} #epsilon_{LB}',len(asymPlusForFill),0.5,len(asymPlusForFill)+0.5), 
                    ROOT.TH1D('plusFillsL','#pi^{+} #epsilon_{LS}',len(asymPlusForFill),0.5,len(asymPlusForFill)+0.5), 
                    ROOT.TH1D('plusFillsU','#pi^{+} #epsilon_{US}',len(asymPlusForFill),0.5,len(asymPlusForFill)+0.5)   ]
                    
    keyList = asymPlusForFill.keys()
    keyList.sort()
    print 'asymPlus summary'
    for i in range(len(asymPlusForFill)):
        tmpY = asymPlusForFill[keyList[i]].GetAsymmetry(0)
        plusHistos[5].SetBinContent(i+1,tmpY.GetBinContent(1))
        plusHistos[5].SetBinError(i+1,tmpY.GetBinError(1))
        
        tmpB = asymPlusForFill[keyList[i]].GetAsymmetry(1)
        plusHistos[6].SetBinContent(i+1,tmpB.GetBinContent(1))
        plusHistos[6].SetBinError(i+1,tmpB.GetBinError(1))
        
        tmpL = asymPlusForFill[keyList[i]].GetAsymmetry(3)
        plusHistos[7].SetBinContent(i+1,tmpL.GetBinContent(1))
        plusHistos[7].SetBinError(i+1,tmpL.GetBinError(1))
        
        tmpU = asymPlusForFill[keyList[i]].GetAsymmetry(4)
        plusHistos[8].SetBinContent(i+1,tmpU.GetBinContent(1))
        plusHistos[8].SetBinError(i+1,tmpU.GetBinError(1))
        
        print '%d     % 1.4f    % 1.4f    % 1.4f    % 1.4f' % (keyList[i],
            float(tmpY.GetBinContent(1)/tmpY.GetBinError(1)),
            float(tmpB.GetBinContent(1)/tmpB.GetBinError(1)),
            float(tmpL.GetBinContent(1)/tmpL.GetBinError(1)),
            float(tmpU.GetBinContent(1)/tmpU.GetBinError(1)))
    
    minusHistos = [ asym_minus.GetAsymmetry(0), asym_minus.GetAsymmetry(1), asym_minus.GetAsymmetry(2), 
                    asym_minus.GetAsymmetry(3), asym_minus.GetAsymmetry(4), 
                    ROOT.TH1D('minusFillsY','#pi^{-} #epsilon_{LY}',len(asymMinusForFill),0.5,len(asymMinusForFill)+0.5), 
                    ROOT.TH1D('minusFillsB','#pi^{-} #epsilon_{LB}',len(asymMinusForFill),0.5,len(asymMinusForFill)+0.5), 
                    ROOT.TH1D('minusFillsL','#pi^{-} #epsilon_{LS}',len(asymMinusForFill),0.5,len(asymMinusForFill)+0.5), 
                    ROOT.TH1D('minusFillsU','#pi^{-} #epsilon_{US}',len(asymMinusForFill),0.5,len(asymMinusForFill)+0.5) ]
    
    keyList = asymPlusForFill.keys()
    keyList.sort()
    print 'asymMinus summary'
    for i in range(len(asymMinusForFill)):
        tmpY = asymMinusForFill[keyList[i]].GetAsymmetry(0)
        minusHistos[5].SetBinContent(i+1,tmpY.GetBinContent(1))
        minusHistos[5].SetBinError(i+1,tmpY.GetBinError(1))
        
        tmpB = asymMinusForFill[keyList[i]].GetAsymmetry(1)
        minusHistos[6].SetBinContent(i+1,tmpB.GetBinContent(1))
        minusHistos[6].SetBinError(i+1,tmpB.GetBinError(1))
        
        tmpL = asymMinusForFill[keyList[i]].GetAsymmetry(3)
        minusHistos[7].SetBinContent(i+1,tmpL.GetBinContent(1))
        minusHistos[7].SetBinError(i+1,tmpL.GetBinError(1))
        
        tmpU = asymMinusForFill[keyList[i]].GetAsymmetry(4)
        minusHistos[8].SetBinContent(i+1,tmpU.GetBinContent(1))
        minusHistos[8].SetBinError(i+1,tmpU.GetBinError(1))
    ...
    makePlots(plusHistos, minusHistos)
            
    #save the histograms to a root file 
    outfile = ROOT.TFile('/Users/kocolosk/Desktop/asymHistos.root','recreate')
    [h.Write() for h in plusHistos]
    [h.Write() for h in minusHistos]
    outfile.Close()


def makePlots(plusHistos, minusHistos):
    plusCanvas = [ROOT.TCanvas('c_plus_%d' % (i,)) for i in range(9)]
    
    minusCanvas = [ROOT.TCanvas('c_minus_%d' % (i,)) for i in range(9)]
    
    plusHistos[0].SetTitle('#pi^{+} #epsilon_{LY}')
    plusHistos[1].SetTitle('#pi^{+} #epsilon_{LB}')
    plusHistos[2].SetTitle('#pi^{+} #epsilon_{LL}')
    plusHistos[3].SetTitle('#pi^{+} #epsilon_{LS}')
    plusHistos[4].SetTitle('#pi^{+} #epsilon_{US}')
    
    [h.SetMarkerStyle(20) for h in plusHistos]
    [h.SetAxisRange(-0.04,0.04,'y') for h in plusHistos[:5]]
    [h.SetAxisRange(-0.10,0.10,'y') for h in plusHistos[5:]]
    [h.SetXTitle('p_{T}') for h in plusHistos[:5]]                
    [h.SetXTitle('fill index') for h in plusHistos[5:]]
                    
    minusHistos[0].SetTitle('#pi^{-} #epsilon_{LY}')
    minusHistos[1].SetTitle('#pi^{-} #epsilon_{LB}')
    minusHistos[2].SetTitle('#pi^{-} #epsilon_{LL}')
    minusHistos[3].SetTitle('#pi^{-} #epsilon_{LS}')
    minusHistos[4].SetTitle('#pi^{-} #epsilon_{US}')
    
    [h.SetMarkerStyle(21) for h in minusHistos]
    [h.SetAxisRange(-0.04,0.04,'y') for h in minusHistos[:5]]
    [h.SetAxisRange(-0.10,0.10,'y') for h in minusHistos[5:]]    
    [h.SetXTitle('p_{T}') for h in minusHistos[:5]]                
    [h.SetXTitle('fill index') for h in minusHistos[5:]]
    
    #time to draw
    for i in range(9):
        plusCanvas[i].cd()
        plusHistos[i].Fit('pol0','q')
        plusCanvas[i].ForceUpdate()
        minusCanvas[i].cd()
        minusHistos[i].Fit('pol0','q')
        minusCanvas[i].ForceUpdate()
        
    saveit = raw_input('Do you want to save the histograms? (y/N): ')
    saveit = saveit.lower()
    if saveit == 'y':
        path = raw_input('Please specify a path (~/): ')
        if path == '': 
            print 'no path given, use home'
            path = '/Users/kocolosk'
        format = raw_input('Please specify a format (gif/eps/png): ')
        if format in ('gif','eps','png'):
            plusCanvas[0].Print('%s/pi_plus_ely_pt.%s' % (path,format))
            plusCanvas[1].Print('%s/pi_plus_elb_pt.%s' % (path,format))
            plusCanvas[2].Print('%s/pi_plus_ell_pt.%s' % (path,format))
            plusCanvas[3].Print('%s/pi_plus_els_pt.%s' % (path,format))
            plusCanvas[4].Print('%s/pi_plus_eus_pt.%s' % (path,format))
            plusCanvas[5].Print('%s/pi_plus_ely_fill.%s' % (path,format))
            plusCanvas[6].Print('%s/pi_plus_elb_fill.%s' % (path,format))
            plusCanvas[7].Print('%s/pi_plus_els_fill.%s' % (path,format))
            plusCanvas[8].Print('%s/pi_plus_eus_fill.%s' % (path,format))
            
            minusCanvas[0].Print('%s/pi_minus_ely_pt.%s' % (path,format))
            minusCanvas[1].Print('%s/pi_minus_elb_pt.%s' % (path,format))
            minusCanvas[2].Print('%s/pi_minus_ell_pt.%s' % (path,format))
            minusCanvas[3].Print('%s/pi_minus_els_pt.%s' % (path,format))
            minusCanvas[4].Print('%s/pi_minus_eus_pt.%s' % (path,format))
            minusCanvas[5].Print('%s/pi_minus_ely_fill.%s' % (path,format))
            minusCanvas[6].Print('%s/pi_minus_elb_fill.%s' % (path,format))
            minusCanvas[7].Print('%s/pi_minus_els_fill.%s' % (path,format))
            minusCanvas[8].Print('%s/pi_minus_eus_fill.%s' % (path,format))
        else: 
            print 'Invalid format specifed!  Histograms will not be saved ...'


def getHistos(tfile):
    print tfile.ls()
    plusHistos = [  tfile.Get('top_plus_0'), tfile.Get('top_plus_1'), tfile.Get('top_plus_2'), 
                    tfile.Get('top_plus_3'), tfile.Get('top_plus_4'), tfile.Get('plusFillsY'), 
                    tfile.Get('plusFillsB'), tfile.Get('plusFillsL'), tfile.Get('plusFillsU') ]
                    
    minusHistos = [ tfile.Get('top_minus_0'), tfile.Get('top_minus_1'), tfile.Get('top_minus_2'), 
                    tfile.Get('top_minus_3'), tfile.Get('top_minus_4'), tfile.Get('minusFillsY'), 
                    tfile.Get('minusFillsB'), tfile.Get('minusFillsL'), tfile.Get('minusFillsU') ]
    return (plusHistos,minusHistos)


def comparisonPlots(tuple5,tuple6):
    plus5 = tuple5[0][2]
    minus5 = tuple5[1][2]
    ...
    leg = ROOT.TLegend(0.15,0.68,0.39,0.87)
    leg.AddEntry(plus5,'2005')
    leg.AddEntry(plus6,'2006 in progress')
    
    leg2 = ROOT.TLegend(0.15,0.68,0.39,0.87)
    leg2.AddEntry(minus5,'2005')
    leg2.AddEntry(minus6,'2006 in progress')
    
    comp0 = ROOT.TCanvas('comp0')
    plus5.SetLineColor(ROOT.kRed)
    plus5.SetMarkerColor(ROOT.kRed)
    plus5.SetTitle('A_{LL} #pi^{+}')
    plus5.GetListOfFunctions().Clear()
    plus5.Scale(float(1/(0.42*0.42)))
    plus5.SetAxisRange(-0.1,0.1,'y')
    plus5.Draw('e1')
    plus6.GetListOfFunctions().Clear()
    plus6.Scale(float(1/(0.6*0.6)))
    plus6.Draw('e1 same')
    leg.Draw()
    
    comp1 = ROOT.TCanvas('comp1')
    minus5.SetLineColor(ROOT.kRed)
    minus5.SetMarkerColor(ROOT.kRed)
    minus5.SetTitle('A_{LL} #pi^{-}')
    minus5.GetListOfFunctions().Clear()
    minus5.Scale(float(1/(0.45*0.45)))
    minus5.SetAxisRange(-0.1,0.1,'y')
    minus5.Draw('e1')
    minus6.GetListOfFunctions().Clear()
    minus6.Scale(float(1/(0.6*0.6)))
    minus6.Draw('e1 same')
    leg2.Draw()
...
def newTest(histDir='/Users/kocolosk/data/run5/hist'):
   allFiles = os.listdir(histDir)
   asym1 = AsymmetryGenerator('asym1')
   asym2 = AsymmetryGenerator('asym2')
   
   scalars = ScalarCounts(os.environ['STAR'] + '/StRoot/StSpinPool/StTamuRelLum/inputs/run5.txt')
   
   for fname in allFiles:
      if not fname.endswith('.root'): continue
      tfile = ROOT.TFile(histDir + '/' + fname)
      mgr = histos.HistogramManager(tfile, ['pt'])
      run = fname[13:20]
      print run
      try:
         bin7 = scalars[run + '-5-7']
         bin8 = scalars[run + '-5-8']
         bin9 = scalars[run + '-5-9']
      except KeyError:
      ...
      asym1.FillFromHistogramManager(mgr, '96233', 1, uu, ud, du, dd, 1.0, 1.0)
      asym2.FillFromHistogramManager(mgr, '96233', -1, uu, ud, du, dd, 1.0, 1.0)
      
   c1 = ROOT.TCanvas('c1')
   asym1.GetAsymmetry('ll').Draw()
   c2 = ROOT.TCanvas('c2')
   asym2.GetAsymmetry('ll').Draw()
   raw_input('balknaqertph')
...
def main(argv=None):
    ...
    try:
        try:
            opts, args = getopt.getopt(argv[1:], "ho:v", ["help", "output=", 'run5', 'run6', 'plot=', 'compare'])
        ...
        for option, value in opts:
            ...
            if option in ("-o", "--output"):
                output = value
            if option in ('--run5'):
                make(initReaderRun5,'/Library/STAR/DEV/extras/StTamuRelLum/inputs/run5.txt','2005',False,'online')
            if option in ('--run6'):
                make(initReaderRun6,'/Library/STAR/DEV/extras/StTamuRelLum/inputs/run6.txt','2006',True,'online')
            if option in ('--plot'):
                tfile = ROOT.TFile(value,'read')
                htuple = getHistos(tfile)
                makePlots(htuple[0], htuple[1])
            if option in ('--compare'):
                tfile5 = ROOT.TFile(args[0],'read')
                tuple5 = getHistos(tfile5)
                tfile6 = ROOT.TFile(args[1],'read')
                tuple6 = getHistos(tfile6)
                comparisonPlots(tuple5,tuple6)
